...
```{r}
messiAndRonaldoLaLigaLong <- read_csv("messiAndRonaldoLaLigaLong.csv")
```
...
output$gap_plot_6 <- renderPlot({
  ggplot(messiAndRonaldoLaLigaLong, aes_string(x = "Season", y = input$y_6, color= "Player")) + 
    geom_point() + 
    geom_smooth(se = FALSE) +
    theme(axis.text=element_text(size=16),
          axis.title=element_text(size=16) )
...
output$hover_data_6 <- renderTable({
   nearPoints(MessiRonaldoLonger, input$hover_coord_6)[,1:6]
  })
```
...
# render plot
shinyServer(output$gap_plot_5 <- renderPlot({
...
shinyServer(output$hover_data_5 <- renderTable({
...
shinyUI(fluidPage(
  fluidRow( 
    ...
    column(width = 8, 
             plotOutput("gap_plot_6",hover="hover_coord_6")
      )
    ),
  fluidRow(
      column(width = 12, 
             tableOutput("hover_data_6")
      )
   )
 )
)
...
# render plot
shinyServer(output$gap_plot_6 <- renderPlot({
  ggplot(messiAndRonaldoLaLiga, aes_string(x = "Season", y = input$y_6, color= "Player")) + 
    geom_point() + 
    geom_smooth(se = FALSE) +
    theme(axis.text=element_text(size=16),
          axis.title=element_text(size=16) )
...
## render text for hovered point(s)
shinyServer(output$hover_data_6 <- renderTable({
   nearPoints(messiAndRonaldoLaLiga, input$hover_coord_6)[,1:5]
  }))
```
...
# render plot
shinyServer(output$gap_plot_7 <- renderPlot({
    ggplot(messiAndRonaldoUEFA, aes_string(x = "Season", y = input$y_7, color= "Player")) + 
      geom_point() + 
      geom_smooth(se = FALSE) +
      theme(axis.text=element_text(size=16),
            axis.title=element_text(size=16) )
...
## render text for hovered point(s)
shinyServer(output$hover_data_7 <- renderTable({
     nearPoints(messiAndRonaldoUEFA, input$hover_coord_7)[,1:5]
    }))
```

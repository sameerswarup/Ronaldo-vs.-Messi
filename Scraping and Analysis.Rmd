...
messiLaLiga <- messiLaLiga %>% select(Season, Club) %>% rename(Goals = "Club") %>% drop_na()
```

```{r}
messiCopaDelRey <-html_table(messiTable.html[[2]], fill = TRUE) %>% as_tibble(.name_repair = "unique")

messiCopaDelRey <- messiCopaDelRey %>% select(Season, Club) %>% rename(Goals = "Club") %>% drop_na()
```

```{r}
messiChampionsLeague <-  html_table(messiTable.html[[3]], fill = TRUE) %>% as_tibble(.name_repair = "unique")

messiChampionsLeague <- messiChampionsLeague %>% select(Season, Club) %>% rename(Goals = "Club") %>% drop_na()
```
...
ronaldoLaLiga <- ronaldoLaLiga %>% select(Season, Club) %>% rename(Goals = "Club") %>% drop_na()
```

```{r}
ronaldoCopaDelRey <-html_table(ronaldoTable.html[[2]], fill = TRUE) %>% as_tibble(.name_repair = "unique")

ronaldoCopaDelRey <- ronaldoCopaDelRey %>% select(Season, Club) %>% rename(Goals = "Club") %>% drop_na()
```

```{r}
ronaldoChampionsLeague <-  html_table(ronaldoTable.html[[3]], fill = TRUE) %>% as_tibble(.name_repair = "unique")

ronaldoChampionsLeague <- ronaldoChampionsLeague %>% select(Season, Club) %>% rename(Goals = "Club") %>% drop_na()
```
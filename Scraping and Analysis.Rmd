...
messiLaLiga <- messiLaLiga %>% select(Season, Club) %>% rename(Goals = "Club") %>% drop_na() %>% slice(3:11)
```
...
ronaldoLaLiga <- ronaldoLaLiga %>% select(Season, Club) %>% rename(Goals = "Club") %>% drop_na() %>% slice(3:11)
```
